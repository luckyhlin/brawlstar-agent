...
| Local→droplet `rsync data/brawlstars.db brawl:...` dies mid-stream with `client_loop: send disconnect: Broken pipe` even with `ServerAliveInterval` keepalives | Path-level connection killer (most likely your residential ISP shaping sustained unidirectional uploads, possibly home-router NAT eviction). Idle SSH holds for 120+ s; sustained data flow dies in seconds regardless of rate. Diagnostic: `dd if=/dev/zero bs=1M count=2048 status=none \| pv -L 10m \| ssh host 'cat > /dev/null'` — observed 2026-05-06 dying in **7 s at 10 MB/s** and **15 s at 1 MB/s** on a Comcast residential → DO path. Rsync sessions (with their bidirectional protocol back-chatter) survived 50–80 s on the same path. SSH keepalives only protect the *idle* case, not active flows. | **Patient `--append-verify` retry loop**. Each retry MD5-resumes from the existing prefix on the droplet. The transfer eventually completes — for the same 15 GB DB on the same residential→DO path it took **112 attempts / 72 min wall clock / ~9 GB net delta after the resume started** before completion. Use `scripts/rsync-db-to-droplet.sh` for the canned version. Inline minimal: `until rsync -avh --append-verify --bwlimit=10M --progress -e 'ssh -o ServerAliveInterval=30 -o TCPKeepAlive=yes -o ConnectTimeout=15' data/brawlstars.db brawl:brawlstar-agent/data/brawlstars.db; do sleep 15; done`. The chunk-and-cat fallback (`scripts/chunk-rsync-to-droplet.sh`) is available but turned out to be over-engineering — the simple retry loop reaches the destination on its own. |
| `rsync brawl:'$HOME/brawlstar-agent/...' local.db` fails with `rsync: [sender] change_dir "/home/<user>/$HOME/brawlstar-agent/data" failed: No such file or directory (2)` — note the literal `$HOME` baked into the path | rsync **3.2.4+** enables `--secluded-args` (formerly `--protect-args`) by default. Path arguments after `host:` are sent straight to the remote `rsync` and **are not interpreted by the remote shell**, so `$HOME` (and any other shell variable) stays literal. Since `$HOME/...` isn't an absolute path, the remote rsync resolves it relative to the SSH user's home → `/home/<user>/$HOME/...`. The companion `ssh "$host" "sqlite3 \$HOME/..."` form keeps working because `ssh "host" "cmd"` DOES go through a remote shell that expands `$HOME`. Affects rsync push and pull symmetrically. Confirmed on rsync 3.2.7 / 2026-05-13. | Use a **relative path** (`brawlstar-agent/data/brawlstars.db`) or a **tilde** (`~/brawlstar-agent/data/brawlstars.db`) on the remote side. Both are handled by rsync itself even with secluded-args, and both also work in the `ssh "$host" "sqlite3 $REMOTE_PATH ..."` shell form (relative paths resolve against the non-interactive ssh CWD = `$HOME`). Already applied to `scripts/rsync-db-{from,to}-droplet.sh`. Workaround for ad-hoc invocations is `--old-args` (or `RSYNC_OLD_ARGS=1`), but that path is being deprecated by rsync — prefer the relative/tilde fix. |
